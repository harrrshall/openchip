...
def check_hdlc(contract: Contract, request: str, reference: Path, work: Path,
    ...
    if not binding:
        return None
    result=dict(status='error',tables=0,rows=0,mismatches=[],checked_kinds=['hdlc_framing'],binding=binding)
    if incomplete:
    ...
    for pattern in range(4096):
        inputs.extend({'in':b} for b in [0]+[(pattern>>i)&1 for i in range(12)]+[0,0])
    expected=[];history='0';previous={'disc':0,'flag':0,'err':0}
    ...
    work.mkdir(parents=True,exist_ok=True);run=Path(tempfile.mkdtemp(prefix='hdlc-',dir=work))
    cp=run/'contract.json';cp.write_text(contract.model_dump_json(indent=1))
    replay=run/'inputs.json';replay.write_text(json.dumps({'inputs':inputs}));(run/'expected.json').write_text(json.dumps(expected))
    ...
    if data.get('error') or len(data.get('outputs',[]))!=len(inputs):
        return {**result,'detail':'HDLC reference replay failed: '+str(data.get('error') or 'incomplete outputs')[:600]}
    failed=0;mismatches=[]
    for i,(want,got) in enumerate(zip(expected,data['outputs'])):
        if want!=got:
            failed+=1
            if len(mismatches)<6:mismatches.append(dict(cycle=i,preceding_inputs=inputs[max(0,i-9):i],request_says=want,reference_says=got))
    result.update(status='mismatch' if failed else 'ok',rows=len(inputs),checked_cycles=len(inputs),patterns=4096,
                  mismatch_vectors=failed,mismatches=mismatches,reference_sha256=hashlib.sha256(reference.read_bytes()).hexdigest(),
    ...
    (run/'result.json').write_text(json.dumps(result,indent=2));return result
