...
def simulation_settings_error(cycles: int, seeds: list[int]) -> str:
    if type(cycles) is not int or cycles < 1:
        return "Simulation requires a positive integer cycle count."
    if not seeds or any(type(seed) is not int for seed in seeds):
        return "Simulation requires at least one integer seed."
    return ""
...
def compare_references(contract: Contract, ref_a: Path, ref_b: Path, work: Path, seeds: list[int], cycles: int) -> dict:
    ...
    error = simulation_settings_error(cycles, seeds)
    if error:
        return {"error": error}
...
def verify(contract: Contract, rtl_path: Path, reference_py: Path, work: Path, cfg: Config,
    ...
    cycles = cfg.verification.sim_cycles if cycles is None else cycles
    seeds = cfg.verification.seeds if seeds is None else seeds
    error = simulation_settings_error(cycles, seeds)
    if error:
        return VerificationResult(stage="configuration", accepted=False, summary=error)
    work.mkdir(parents=True, exist_ok=True)
...
def parse_sim(sim: ToolResult, seed: int, cycles: int, vec: dict) -> SimSeedResult:
    if type(cycles) is not int or cycles < 1:
        return SimSeedResult(seed, cycles, "sim_error", detail="No positive simulation observation count.")
