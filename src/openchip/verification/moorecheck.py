...
def check_moore(contract: Contract, request: str, reference: Path, work: Path,
    ...
    result = dict(status='error', tables=0, rows=0, mismatches=[],
                  checked_kinds=['moore_transition_table'], binding=binding)
    if incomplete:
    ...
    while queue:
        ...
        for bit in (0, 1):
            ...
            if target not in paths:
                paths[target] = paths[state] + [bit]
                queue.append(target)
    work.mkdir(parents=True, exist_ok=True)
    run = Path(tempfile.mkdtemp(prefix='moore-', dir=work))
    ...
    for index, (state, path) in enumerate(paths.items()):
        for bit in (0, 1):
            ...
            inputs = [{'in': value} for value in path + [bit, 0]]
            current = initial
            expected = []
            for vector in inputs:
                expected.append({'out': rows[current][2]})
                current = rows[current][vector['in']]
            replay = run / f'{index}-{bit}-inputs.json'
            ...
            data = run_reference(reference, contract_path, 0, len(inputs),
                                 run / f'{index}-{bit}-reference.json',
                                 python=python, timeout_s=remaining, replay=replay)
            if data.get('error') or len(data.get('outputs', [])) != len(inputs):
                return {**result, 'detail': 'Moore reference replay failed: ' +
                        str(data.get('error') or 'incomplete output')[:600]}
            for cycle, (wanted, actual) in enumerate(zip(expected, data['outputs'])):
                observations += 1
                if wanted != actual:
                    failed += 1
                    if len(mismatches) < 6:
                        mismatches.append(dict(target_state=state, transition_input=bit, cycle=cycle,
    ...
    result.update(status='mismatch' if failed else 'ok', rows=observations, checked_cycles=observations,
                  reachable_states=len(paths), checked_transitions=2 * len(paths),
