...
READY/VALID STALL SAMPLING: if the specified protocol requires valid/data to hold until a transfer, a direct hold assertion is about the PREVIOUS edge's handshake. For a synchronous-reset source interface, save prev_out_valid, prev_out_ready, prev_out_data and normalized prev_reset_active together. Once history is valid, use `if (prev_out_valid && !prev_out_ready && !prev_reset_active) begin assert(out_valid); assert(out_data == prev_out_data); end`. Never combine prev_out_valid with CURRENT out_ready: ready may change between observations, and the current value cannot tell whether the preceding edge transferred a byte. Assert before nonblocking updates; then save CURRENT observations. Separately use CURRENT ready/valid inputs for shadow-state transitions and CURRENT inputs for combinational ready decoding. Do not assume ready stays stable. Gate unknown history, adapt port names/widths/polarity, and apply this hold obligation only when the contract specifies it. This synchronous-reset example does not handle asynchronous reset or protocols permitting valid withdrawal unchanged. Current synchronous reset affects the upcoming update; previous reset activity determines the state now observed.

For a reconstructed FSM, output assertions decode the CURRENT shadow state, not a transition about to happen. If the shadow is still in capture on the edge consuming the last payload bit, a registered counting output is still inactive inside the assertion; it becomes active when the updated shadow is in counting at the next observation. Likewise, a terminal counter value in the counting state does not make the completion-state output visible before the transition edge. Assert the specified outputs by decoding the CURRENT shadow state, then update the shadow state from CURRENT inputs. Do not accidentally add an output pipeline stage: for Moore outputs with no separately specified extra latency, `expected <= decode(old_shadow_state)` alongside `shadow_state <= next_shadow_state` makes expected one cycle late. Prefer asserting the decode directly; if a separate expected register is necessary, update it from `decode(next_shadow_state)` on the SAME edge as the state update, with the same reset priority. These decode/next-state names illustrate scheduling; implement the actual contract logic. Preserve any additional latency explicitly required by the request. A detection state is sticky only when the request says detection persists until reset, not merely because of its name. Never use current incoming payload bits to predict already-registered output bits. Trace the last capture edge, first counting observation, last counting observation, and first completion observation separately.
