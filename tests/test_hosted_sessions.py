import json, tempfile, threading, http.client
...
def test_http_session_isolation_and_key_retention(tmp_path, monkeypatch):
    ...
    a.workspaces.mkdir(); ws = Workspace(a.workspaces/'private-design'); ws.init(request='private hardware design request', name='private-design')
    ...
    try: b.model_adapter(b.config())
    except ValueError: checks.append('no owner credential fallback')
    else: raise AssertionError('owner fallback')
