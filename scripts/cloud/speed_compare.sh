...
cd /home/openchip
OUT=/home/openchip-runs/evals/speed-compare
mkdir -p $OUT
for ENVF in "$@"; do
  cp "$ENVF" /home/openchip-env/model.env
  ...
  a="vllm ser"
  pkill -f "${a}ve" 2>/dev/null
  sleep 6
  nohup bash /home/openchip-env/serve.sh > /home/openchip-runs/vllm/serve-$SLUG.out 2>&1 &
  for i in $(seq 1 120); do
    curl -sf -H "Authorization: Bearer $OPENCHIP_MODEL_API_KEY" http://127.0.0.1:8000/v1/models 2>/dev/null | grep -q "$OPENCHIP_MODEL" && break
    sleep 10
  done
  ...
  d=$(ls -dt $OUT/core-v1-$SLUG-* | head -1)
  python3 - "$d" <<PY
...
echo "== speed compare done $(date -u +%T)"
touch $OUT/DONE
