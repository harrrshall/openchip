...
. "$ROOT/secrets.env"; . "$ROOT/model.env"; export OPENCHIP_MODEL_API_KEY OPENCHIP_MODEL OPENCHIP_MODEL_REVISION OPENCHIP_THINKING_ROLES OPENCHIP_EXTRA_BODY
SLUG=$(echo "${OPENCHIP_MODEL##*/}" | tr -c 'A-Za-z0-9.\n' '-')
LOG=/home/openchip-runs/bench-$SLUG.log; mkdir -p /home/openchip-runs/evals
...
. "$ROOT/env.sh"
cd /home/openchip
[ -x .venv/bin/openchip ] || { uv venv .venv --python 3.12 -q && uv pip install --python .venv/bin/python -q -e ".[dev]"; }
...
if [ "$SERVED" != "\"id\":\"$OPENCHIP_MODEL\"" ]; then
  ...
  for i in $(seq 1 240); do curl -sf -H "Authorization: Bearer $OPENCHIP_MODEL_API_KEY" http://127.0.0.1:8000/v1/models >/dev/null && break; sleep 10; done
fi
