...
[ -f /home/openchip-env/secrets.env ] && . /home/openchip-env/secrets.env
PROVIDER=$1
BASE=$2
MODEL=$3
KEYENV=$4
UA=${5:-}
OUT=${6:-/home/openchip-runs/evals/remote}
...
FROM=${BENCH_FROM:-core-v1}
STARTED=0
run() {
  [ "$1" = "$FROM" ] && STARTED=1
  if [ $STARTED = 0 ]; then
    echo "== skip $1" | tee -a "$LOG"
    return
  fi
...
echo "== bench_remote done $(date -u +%FT%TZ)" | tee -a "$LOG"
touch "$OUT/DONE-$SLUG"
